...
export function setupAgentRoutes(app: FastifyInstance) {
  ...
  app.put("/api/agents/:id", async (req, reply) => {
    ...
    const body = req.body as Partial<{ name: string; description: string; systemPrompt: string; icon: string; model: string; disabledTools: string[]; enabledMcpServers: string[]; enabledSkills: string[] }> | null;
